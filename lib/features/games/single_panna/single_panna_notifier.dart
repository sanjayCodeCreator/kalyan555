...
final singlePannaNotifierProvider =
    AsyncNotifierProvider.autoDispose<SinglePannaNotifier, SinglePannaMode>(() {
  return SinglePannaNotifier();
});

class SinglePannaMode {
...
class SinglePannaNotifier extends AutoDisposeAsyncNotifier<SinglePannaMode> {
  final SinglePannaMode _outputMode = SinglePannaMode();
  ...
  void addPoints(BuildContext context) async {
    try {
      ...
      if (true) {
        if (!singlePannaList.contains(_outputMode.enteredNumber.text)) {
      ...
      _outputMode.singlePannaModel.first = SinglePannaModel(
