...
final halfSangamNotifierProvider =
    AsyncNotifierProvider.autoDispose<HalfSangamNotifier, HalfSangamMode>(() {
  return HalfSangamNotifier();
});

class HalfSangamMode {
  ...
  bool isOpenDigitClosePana = true;
  int? startDigit = 1;
...
class HalfSangamNotifier extends AutoDisposeAsyncNotifier<HalfSangamMode> {
  final HalfSangamMode _outputMode = HalfSangamMode();

  void changeOpenDigitClosePanna(bool value) {
    _outputMode.isOpenDigitClosePana = value;
  ...
  void addPoints(BuildContext context) async {
    try {
      ...
      if (!(_outputMode.closePana
          .contains(_outputMode.closePanaController?.text))) {
        if (context.mounted) {
      ...
      if (_outputMode.openDigitController!.text.isNotEmpty &&
          _outputMode.closePanaController!.text.isNotEmpty &&
          _outputMode.pointsController!.text.isNotEmpty) {
        _outputMode.selectedOpenList.add(_outputMode.openDigitController!.text);
  ...
  void clearAddNumberList() {
    try {
      _outputMode.openDigitController?.text = '';
    ...
    if (_outputMode.selectedOpenList.isNotEmpty &&
      ...
      try {
          ...
          for (int i = 0; i < _outputMode.selectedCloseList.length; i++)
            {
              ...
              "session": "close",
              "tag": tag,
              "open_digit": _outputMode.isOpenDigitClosePana
                  ? _outputMode.selectedOpenList[i].toString()
                  : "-",
              "close_digit": _outputMode.isOpenDigitClosePana
                  ? "-"
                  : _outputMode.selectedOpenList[i].toString(),
              "open_panna": _outputMode.isOpenDigitClosePana
                  ? "-"
                  : _outputMode.selectedCloseList[i].toString(),
              "close_panna": _outputMode.isOpenDigitClosePana
                  ? _outputMode.selectedCloseList[i].toString()
                  : "-",
              "points": int.parse(_outputMode.selectedPointsList[i].toString()),
              "game_mode": "half-sangam",
              "market_id": marketId
            },
        ];

        log(jsonDataArray.toList().toString(), name: 'half-sangum');
        ...
        if (singlePanaModel?.status == "success") {
          ...
          // if (context.mounted) {
          //   context.pushReplacementNamed(HomePath.homeScreen);
        ...
        EasyLoading.dismiss();
        log(e.toString(), name: 'half-sangum');
      }
    } else {
      toast(context: context, 'Please select number and add points');
    }
...
class SelectedHalfSangamModel {
  ...
  String? points;
  bool? isClosed;
  SelectedHalfSangamModel({this.points, this.value, this.isClosed});
}
