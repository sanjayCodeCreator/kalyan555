...
import 'package:sm_project/features/games/triple_panna_bulk/triple_panna_bulk_notifier.dart';
...
class TriplePanaBulkScreen extends HookConsumerWidget {
  final String id;
  final String tag;
  final String name;
  const TriplePanaBulkScreen({
    super.key,
    required this.id,
    required this.tag,
    required this.name,
            ...
            child: InkWell(
              onTap: () async {
                ...
                if (isStopGameExecution(
                  ...
                  ref: ref,
                  marketId: id,
                  gameName: "Triple Panna Bulk",
                )) {
                  return;
                }
                if (context.mounted) {
                  ref
                      .read(triplePanaBulkNotifierProvider.notifier)
                    ...
                    Consumer(
                      builder: (context, ref, child) {
                        final refWatch = ref.watch(
                          triplePanaBulkNotifierProvider,
                        );
                        final refRead = ref.read(
                          triplePanaBulkNotifierProvider.notifier,
                        );
                        return Container(
                          padding: const EdgeInsets.all(8.0),
                              ...
                              child: Consumer(
                                builder: (context, ref, child) {
                                  final selectedNumbers = ref
                                          .watch(triplePanaBulkNotifierProvider)
                                          .value
                                          ?.selectedNumberList ??
                                      [];
  ...
  Widget chooseNumber(BuildContext context, String number) {
    return Consumer(
      builder: (context, ref, _) {
        final refWatch = ref.watch(triplePanaBulkNotifierProvider).value;
        final refRead = ref.read(triplePanaBulkNotifierProvider.notifier);
        return InkWell(
          onTap: () async {
