...
class RouletteWinHistoryScreen extends StatefulWidget {
  final String userId;

  const RouletteWinHistoryScreen({
    super.key,
    required this.userId,
  });

  @override
  State<RouletteWinHistoryScreen> createState() =>
      _RouletteWinHistoryScreenState();
}

class _RouletteWinHistoryScreenState extends State<RouletteWinHistoryScreen> {
  final RouletteApiService _apiService = RouletteApiService();
  List<RouletteBidModel> _winHistory = [];
  ...
  Future<void> _fetchWinHistory() async {
    try {
      String? fromString;
      String? toString;
      ...
      final wins = await _apiService.getBets(
      ...
      debugPrint('Error fetching win history: $e');
      setState(() {
        _isLoading = false;
      });
    }
          ...
          Expanded(
            child: _isLoading
                ...
                : _winHistory.isEmpty
                    ...
                    : RefreshIndicator(
                        onRefresh: _fetchWinHistory,
                        color: const Color(0xFFD9BC4C),
                        child: ListView.builder(
                          padding: const EdgeInsets.all(8),
                          itemCount: _winHistory.length,
                          itemBuilder: (context, index) {
                            final win = _winHistory[index];
                            return Card(
                              elevation: 4,
